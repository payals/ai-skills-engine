...
1. Ensure to run all relevant tests to verify that everything works as expected and that no existing functionality is broken.
2. Assume you are a QA engineer and think about edge cases and potential issues that might arise from the new feature or change. Write additional tests if necessary to cover these scenarios. Document any concerns and assumptions
3. Update any documentation files to reflect the new feature or change. This includes code comments, README files, and any other relevant documentation.
4. **Update IMPLEMENTATION_PLAN.md or IMPLEMENTATION_PLAN_V3.md**: Mark completed tasks, add session notes in the "Session Handoff Notes" section, and update blockers if applicable in the file that is being referenced.

Avoid code duplication by reusing existing functions and modules wherever possible. Refactor code to improve readability and maintainability, adhering to the project's coding standards and best practices.

When running anything in terminal, make sure .venv is activated for the project